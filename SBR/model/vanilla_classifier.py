    ...
    def forward(self, batch):
        ...
        if True:
            ...
            for ch in range(num_chunks):
                ...
                output = self.bert.forward(input_ids=torch.concat([torch.ones((batch_size, 1), dtype=torch.int64) * CLS_token, batch['item_input_ids'][:, start:end]], dim=1),
                                           attention_mask=torch.concat([torch.ones((batch_size, 1), dtype=torch.int64), batch['item_attention_mask'][:, start:end]], dim=1))
                if self.agg_strategy == "CLS":
                    temp = output.pooler_output
                elif self.agg_strategy == "mean":
                    raise ValueError("not implemented yet")
