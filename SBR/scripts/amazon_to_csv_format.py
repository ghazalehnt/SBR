...
def main(interaction_input_file, item_meta_input_file, output_path, database_name):
    ...
    with gzip.open(interaction_input_file) as f:
        for line in f:
            ...
            user_names[r["reviewerID"]] = r["reviewerName"]
    ...
    with open(join(output_path, f"{database_name}.users"), 'w') as f:
        ...
        writer.writerows([[k, v] for k, v in user_names.items()])
...
if __name__ == '__main__':
    ...
    main(join(input_folder, inter_file), join(input_folder, item_meta_file), output_folder)