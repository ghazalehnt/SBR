...
def main():
    ...
    while len(final_selected_users) < total_num_users:
        ...
        if su_objective == "random":
            su_degree = {user: 1 for user in user_item_degree if user not in final_selected_users}
        elif su_objective == "dense":
            su_degree = {user: user_item_degree[user] for user in user_item_degree if user not in final_selected_users}
        elif su_objective == "sparse":
            su_degree = {user: (1/user_item_degree[user]) for user in user_item_degree if user not in final_selected_users}
        else:
        ...
        if u_objective == "random":
            ...
            h1_users_degree = {u: user_item_degree[u] for u in h1_users}
        elif u_objective == "sparse":
            h1_users_degree = {u: (1/user_item_degree[u]) for u in h1_users}
        else:
            raise ValueError("Not implemented")
    ...
    OUTPUT_DATASET = join(DATASET_PATH,
                          ...
                          f'user-propag-{user_propagation_number}_{u_objective}_degree')
