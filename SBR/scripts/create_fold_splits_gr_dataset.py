...
def get_per_rating_interactions_cnt(inters):
    rating_inters = {'did not like it': 0, 'it was ok': 0, 'liked it': 0, 'really liked it': 0, 'it was amazing': 0}
    for line in inters:
        rating = line[3]
        rating_inters[rating] += 1
...
if __name__ == '__main__':
    ...
    RAW_DATASET_PATH = "TODO"
    OUT_DATASET_PATH = "TODO"
    # OUT_DATASET_PATH = "data"
    ...
    if rating_threshold is None:
        OUTPUT_DATASET = f"{OUT_DATASET_PATH}/GR_read_{num_folds}-folds/"
    else:
        OUTPUT_DATASET = f"{OUT_DATASET_PATH}/GR_rating{rating_threshold}_{num_folds}-folds/"
    ...
    per_rating_interactions_cnt = get_per_rating_interactions_cnt(interactions)
