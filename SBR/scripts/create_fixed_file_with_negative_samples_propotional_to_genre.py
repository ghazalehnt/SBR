            ...
            if remaining_num_samples > 0:
                for g in [g for g, v in sorted(per_user_train_genre_cnt[user_id].items(), key=lambda x: x[1], reverse=True)]:
                    ...
                    if sum(num_samples_per_genre.values()) == remaining_num_samples:
                        break
                #     num_samples_per_genre[g] = int(round_half_up((per_user_train_genre_cnt[user_id][g]
                #                                                   * remaining_num_samples) / num_total_genres))

        neg_samples.extend([[user_id, item_id, 0] for item_id in user_samples])
    return neg_samples


def main(dataset_path, strategy, num_neg_samples, genre_field):
    ...
    validation_samples = neg_sampling_by_train_genre(datasets['validation'], per_user_train_genre_cnt, item_by_genre,
    ...
    test_samples = neg_sampling_by_train_genre(datasets['test'], per_user_train_genre_cnt, item_by_genre, used_items,
                                               num_neg_samples)
