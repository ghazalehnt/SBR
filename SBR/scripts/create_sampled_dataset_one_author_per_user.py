...
if __name__ == "__main__":
    ...
    with open(join(DATASET_PATH, ITEM_FILE), 'r') as f:
        ...
        for line in reader:
            ...
            tie_brk_line = line[TIE_BREAK_IDX_ITEM]
            if INTERACTION_FILE.startswith("amazon_reviews_books"):
                if tie_brk_line in ["", "[]"]:
                    pass
                elif "in" in tie_brk_line:
                    # in Books
                    try:
                        tie_brk = int(re.sub(r"(?:\['>#)?([\d,]+) in Books.*", '\g<1>', tie_brk_line).replace(',', ''))
                        item_tie_breaks[item_id][0] = tie_brk
                    except:
                        try:
                            tie_brk = int(
                                re.sub(r"(?:\['>#)?([\d,]+) Paid in Kindle Store.*", '\g<1>', tie_brk_line).replace(',', ''))
                            item_tie_breaks[item_id][1] = tie_brk
                        except:
                            try:
                                tie_brk = int(
                                    re.sub(r"(?:\['>#)?([\d,]+) Free in Kindle Store.*", '\g<1>', tie_brk_line).replace(',', ''))
                                item_tie_breaks[item_id][2] = tie_brk
                            except:
                                try:
                                    tie_brk = int(
                                        re.sub(r"(?:\['>#)?([\d,]+) in .*", '\g<1>', tie_brk_line).replace(',', ''))
                ...
                if tie_brk_line in [""]:
                    ...
                    item_tie_breaks[item_id] = float(tie_brk_line)
