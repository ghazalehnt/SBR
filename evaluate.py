...
def get_metrics(ground_truth, prediction_scores, weighted_labels, ranking_metrics=None):
    ...
    results = calculate_ranking_metrics_macro_avg_over_users(gt=ground_truth, pd=prediction_scores,
                                                             ...
                                                             weighted_label=weighted_labels)
...
def main(config, valid_gt, valid_pd, test_gt, test_pd, thresholds,
    ...
    rows_valid = []
    rows_test = []
    rows_valid.append(["group"] + ranking_metrics)
    rows_test.append(["group"] + ranking_metrics)
    # ALL:
    ...
    rows_valid.append(["Valid - ALL"] + [valid_results[h] for h in ranking_metrics])
    ...
    rows_test.append(["Test - ALL"] + [test_results[h] for h in ranking_metrics])
    ...
    for gr in user_groups:
        ...
        rows_valid.append([f"Valid - group {gr}"] + [valid_results[h] if h in valid_results else "" for h in ranking_metrics])
        ...
        rows_test.append([f"Test - group {gr}"] + [test_results[h] if h in test_results else "" for h in ranking_metrics])
