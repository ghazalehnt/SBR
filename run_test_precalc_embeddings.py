	...
	for user_exid, user_inid in user_id_mapping.items():
		...
		for item_inid in range(len(res)):
			if item_inid in not_include_in_test[user_inid]:
				continue
			temp[item_id_mapping_rev[item_inid]] = res[item_inid]
		results[user_exid] = {k: v for k, v in sorted(temp.items(), key=lambda x: x[1], reverse=True)[:topN]}
		if len(results.keys()) % 1000 == 0:
			print(f"{len(results.keys())} users done.")

	json.dump(results, open(join(result_folder, f"scores_sorted_{topN}_of_all_items_per_user.json"), 'w'))
